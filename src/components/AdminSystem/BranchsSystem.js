...
/*
  const fetchPOO = async ()=>{
    try{
     const response = await fetch("http://secondsin-001-site1.dtempurl.com/tablePOO/", {
       method: "get",
  ...
  return (
    <div>
      ...
      <div className='branchs'>
        <div>
          <span onClick={()=>{setOpenRespublic(!openRespublic); setOpenOblast(false); setOpenRaion(false); setLabelYzel('Республика Беларусь')}}>Республика Беларусь</span>
        </div> 
